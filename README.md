...
<!--
  DEMO VIDEO / GIF
  Record a 30–60s screen capture: (1) drag the simulator sliders and watch the gauge react,
  (2) run through the four-step workflow tab, (3) show `npm test` passing in a terminal.
  Save it as docs/demo.gif (or upload to GitHub and grab the asset URL), then uncomment below.
...
- An application has exactly one decision
...
Plain HTML, CSS, and vanilla JavaScript — no build step, no framework. Two tabs:
